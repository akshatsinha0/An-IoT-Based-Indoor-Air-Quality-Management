...
% Data Sources Layer
\node[databox] (dataset) {LBNL Indoor IAQ\\Dataset (CSV)};
\node[databox, right=0.5cm of dataset] (simulator) {Simulator\\(Python)};
...
For realistic data validation and testing, we utilize the \textbf{LBNL Indoor Air Quality Dataset} from Lawrence Berkeley National Laboratory (available at https://iaqdata.lbl.gov/), which contains real-world indoor air quality measurements from residential homes, offices, and educational facilities. This dataset is specifically designed for indoor environmental quality research and includes the exact parameters our system monitors. The dataset includes:
\begin{itemize}
  \item CO\textsubscript{2} (Carbon Dioxide) concentration in ppm - indicating ventilation effectiveness
  \item PM2.5 (Particulate Matter ≤2.5 µm) in µg/m\textsuperscript{3} - critical for respiratory health
  \item Temperature in °C - affecting thermal comfort
  \item Relative Humidity in \% - influencing comfort and mold growth
  \item VOCs (Volatile Organic Compounds) in ppb - from building materials and activities
  \item Timestamps with hourly or sub-hourly resolution
\end{itemize}

The LBNL dataset is ideal for our project because it represents actual indoor environments (not outdoor urban air), includes PM2.5 measurements essential for CPCB compliance testing, covers multiple building types matching our site profiles (Lab, Classroom, Canteen), and provides research-grade sensor data with documented calibration procedures. We use this dataset to validate our system's data ingestion pipeline, test CPCB PM2.5 categorization accuracy against real indoor pollution levels, benchmark our exposure time tracking algorithms, and demonstrate multi-site monitoring with authentic indoor air quality patterns. This approach ensures our software-based system can seamlessly transition to real sensor deployments while maintaining compatibility with India's CPCB air quality standards for PM2.5 monitoring.
