...
\centering
\vspace*{0.5cm}
{\large A\\[6pt] Final Project Report\\[6pt] Submitted}\\[10pt]
{\bfseries\Large in Fulfillment of the Requirements}\\[8pt]
{\itshape for the course}\\[4pt]
{\Large \course}\\[10pt]

{\itshape on the project entitled}\\[4pt]
{\bfseries\LARGE \projecttitle}\\[10pt]
...
% Course Details Box
\begin{tikzpicture}
\node[draw, rounded corners, fill=blue!5, text width=10cm, align=center, inner sep=8pt] {
  \small
  \textbf{Class ID:} VL2025260101707 \quad | \quad \textbf{Slot:} A2+TA2\\[2pt]
  \textbf{Classroom:} SJT504
};
\end{tikzpicture}
...
\subsection{CPCB PM2.5 Sub-Index Calculation Algorithm}
The core innovation lies in our implementation of the CPCB National Air Quality Index calculation for PM2.5, which uses piecewise linear interpolation across defined breakpoints. The mathematical formulation is:

\begin{equation}
I_{PM2.5} = \frac{I_{hi} - I_{lo}}{BP_{hi} - BP_{lo}} \times (C_{PM2.5} - BP_{lo}) + I_{lo}
\end{equation}

where $I_{PM2.5}$ is the PM2.5 sub-index, $C_{PM2.5}$ is the measured PM2.5 concentration (µg/m\textsuperscript{3}), $BP_{lo}$ and $BP_{hi}$ are the breakpoint concentrations, and $I_{lo}$ and $I_{hi}$ are the corresponding index values.

\begin{algorithm}[H]
\caption{CPCB PM2.5 Sub-Index and Exposure Tracking}
\begin{algorithmic}[1]
\Require PM2.5 concentration $C$, timestamp $t$, site identifier $s$
\Ensure Sub-index $I$, category $cat$, exposure update

\State \textbf{Define} CPCB breakpoints:
\State $BP \gets [(0,30,0,50,\text{``Good''}), (31,60,51,100,\text{``Satisfactory''}),$ 
\State \hspace{2.5em} $(61,90,101,200,\text{``Moderate''}), (91,120,201,300,\text{``Poor''}),$ 
\State \hspace{2.5em} $(121,250,301,400,\text{``Very Poor''}), (251,350,401,500,\text{``Severe''})]$

\State \textbf{Function} \textsc{CalculateSubIndex}$(C)$:
\For{each $(BP_{lo}, BP_{hi}, I_{lo}, I_{hi}, cat)$ in $BP$}
    \If{$BP_{lo} \leq C \leq BP_{hi}$}
        \State $I \gets \left\lfloor \frac{I_{hi} - I_{lo}}{BP_{hi} - BP_{lo}} \times (C - BP_{lo}) + I_{lo} \right\rceil$
        \State \Return $(I, cat)$
    \EndIf
\EndFor
\State \textit{// Handle values beyond scale}
\State $I \gets \max(I_{hi}, \text{computed value})$
\State \Return $(I, \text{``Severe''})$

\State
\State \textbf{Function} \textsc{UpdateExposure}$(cat, t, s, window)$:
\State $readings \gets$ \textsc{GetReadings}$(s, t - window, t)$
\State Sort $readings$ by timestamp
\For{$i \gets 1$ to $|readings|$}
    \State $\Delta t \gets readings[i].timestamp - readings[i-1].timestamp$
    \State $exposure[readings[i].category] \gets exposure[readings[i].category] + \Delta t$
\EndFor
\State \Return $exposure$ \textit{// minutes in each category}

\State
\State \textbf{Main Processing:}
\State $(I, cat) \gets$ \textsc{CalculateSubIndex}$(C)$
\State \textsc{StoreReading}$(t, C, I, cat, s)$
\If{$cat \in \{\text{``Poor''}, \text{``Very Poor''}, \text{``Severe''}\}$}
    \State $severity \gets$ ($cat = \text{``Poor''}$) ? ``warning'' : ``critical''
    \State \textsc{GenerateAlert}$(t, s, cat, severity, C)$
\EndIf
\State $exposure_{24h} \gets$ \textsc{UpdateExposure}$(cat, t, s, 24\text{ hours})$
\State $exposure_{7d} \gets$ \textsc{UpdateExposure}$(cat, t, s, 7\text{ days})$
\end{algorithmic}
\end{algorithm}

This algorithm ensures real-time CPCB compliance, automatic alert generation for hazardous conditions, and cumulative exposure tracking across multiple time windows, providing a comprehensive view of indoor air quality that goes beyond instantaneous measurements.
...
Pangestu, A., Yusro, M., Djatmiko, W., \& Jaenul, A. (2020). The monitoring system of indoor air quality based on Internet of Things. In \emph{Proceedings of the 2020 International Seminar on Intelligent Technology and Its Applications (ISITIA)} (pp. 95--100). IEEE. https://www.researchgate.net/publication/344020040\_THE\_MONITORING\_SYSTEM\_OF\_INDOOR\_AIR\_QUALITY\_BASED\_ON\_INTERNET\_OF\_THINGS
...
The complete source code, documentation, and dataset for this project are available on GitHub:

\noindent\url{https://github.com/akshatsinha0/An-IoT-Based-Indoor-Air-Quality-Management}

The repository includes:
\begin{itemize}
  \item Backend API implementation (FastAPI)
  \item Frontend dashboard (Streamlit)
  \item Data simulator and site-specific profiles
  \item UCI Air Quality Dataset
  \item Installation and setup instructions
  \item API documentation
\end{itemize}

\subsection{Live Deployments}
The project has been deployed and is accessible online through the following links:

\subsubsection{Air Quality Forecasting Platform}
\noindent\url{https://airly-vision-forecast-opal.vercel.app/}

A comprehensive AI-powered air quality analysis and forecast platform featuring:
\begin{itemize}
  \item Real-time monitoring from 40 stations across Delhi
  \item 24-hour AQI forecasts using hybrid ML models (Random Forest + BiGRU-GCN)
  \item Interactive maps with pollution hotspots
  \item Health risk assessments and correlation analysis
  \item Built with React, TypeScript, Vite, and Mapbox
\end{itemize}

\subsubsection{IAQ Monitoring Dashboard}
\noindent\url{https://akshatsinha0-an-iot-based-indoor-air-qu-appstreamlit-app-5swpqv.streamlit.app/}

The Streamlit-based indoor air quality monitoring dashboard featuring:
\begin{itemize}
  \item Real-time multi-site monitoring (Lab, Classroom, Canteen)
  \item CPCB-compliant PM2.5 categorization and alerts
  \item Time-series visualization with EWMA smoothing
  \item Exposure time tracking across pollution zones
  \item Interactive charts and real-time metrics
\end{itemize}

\subsection{Project Presentation}
The project presentation slides are available online:

\noindent\url{https://docs.google.com/presentation/d/13Jl-IwKPUhAPIwl7KkVz1MBsobmaXAXQyNOnhCqoDno/edit?usp=sharing}

The presentation covers:
\begin{itemize}
  \item Problem statement and motivation
  \item System architecture and design
  \item Implementation details and features
  \item Performance evaluation and results
  \item Future enhancements and roadmap
\end{itemize}
