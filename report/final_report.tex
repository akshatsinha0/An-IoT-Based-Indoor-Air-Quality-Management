...
% Data Sources Layer
\node[databox] (dataset) {Sample Indoor IAQ\\Data (CSV)};
\node[databox, right=0.5cm of dataset] (simulator) {Parametric\\Simulator};
...
\subsection{Data Sources and Simulation Approach}
Our system employs a hybrid data approach combining parametric simulation with sample indoor air quality data. Since our project focuses on software architecture and CPCB compliance algorithms rather than hardware deployment, we generate realistic indoor air quality patterns using site-specific environmental models validated against published research.

\textbf{Parametric Simulation Model:} We developed three distinct site profiles (Lab, Classroom, Canteen) based on indoor air quality characteristics documented in peer-reviewed literature~\cite{marques2016,mumtaz2021,saini2020}. Each profile incorporates:
\begin{itemize}
  \item \textbf{Baseline concentrations:} Lab (PM2.5: 5--25 µg/m\textsuperscript{3}, CO\textsubscript{2}: 400--800 ppm), Classroom (PM2.5: 20--60 µg/m\textsuperscript{3}, CO\textsubscript{2}: 600--1200 ppm), Canteen (PM2.5: 40--150 µg/m\textsuperscript{3}, CO\textsubscript{2}: 800--1500 ppm)
  \item \textbf{Temporal patterns:} Diurnal cycles reflecting occupancy, cooking activities, and ventilation schedules
  \item \textbf{Stochastic variation:} Gaussian noise models simulating sensor measurement variability
  \item \textbf{Environmental coupling:} Temperature (18--35°C) and humidity (20--80\% RH) correlations
\end{itemize}

\textbf{Sample Indoor Data:} We include a curated sample dataset (\texttt{sample\_indoor\_iaq.csv}) containing representative indoor air quality measurements for four site types (Office, Laboratory, Canteen, Classroom) with hourly resolution. This dataset demonstrates the system's ability to ingest external CSV data and serves as a validation reference for our CPCB categorization algorithms.

\textbf{CPCB Compliance:} All generated PM2.5 values are mapped to CPCB National Air Quality Index categories using official breakpoints: Good (0--30), Satisfactory (31--60), Moderately Polluted (61--90), Poor (91--120), Very Poor (121--250), Severe (251--350 µg/m\textsuperscript{3}). This ensures our system accurately reflects India-specific air quality standards and health advisories.

This simulation-based approach enables comprehensive testing of our monitoring system's data pipeline, alert generation, exposure tracking, and visualization capabilities without requiring physical sensor hardware, making the project accessible for educational and demonstration purposes while maintaining technical rigor.
